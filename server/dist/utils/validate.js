...
function validateMove(move, board, currentPlayer) {
    ...
    const validatePawnMove = () => {
        if (adjustedDirection === "F")
            return isInBounds(row - 1, col) && board[row - 1][col] === '';
        if (adjustedDirection === "B")
            return isInBounds(row + 1, col) && board[row + 1][col] === '';
        if (adjustedDirection === "L")
            return isInBounds(row, col - 1) && board[row][col - 1] === '';
        if (adjustedDirection === "R")
            return isInBounds(row, col + 1) && board[row][col + 1] === '';
        if (adjustedDirection === "FL")
            return isInBounds(row - 1, col - 1) && board[row - 1][col - 1] === '';
    ...
    const validateHero1Move = () => {
        ...
        return validMoves.some(([r, c]) => isInBounds(r, c) &&
